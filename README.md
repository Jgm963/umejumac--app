...
<body class="p-4 md:p-8">
    ...
    <script type="module">
        ...
        const API_KEY = ""; // La clave se proporciona automáticamente en runtime si es necesario
        ...
        // const modalActionsEl = document.getElementById('modal-actions'); // No usado pero mantenido
        ...
        onAuthStateChanged(auth, (user) => {
            ...
            if (user) {
                ...
                // Iniciar la escucha de productos solo después de la autenticación
                listenToProducts(); 
            } else {
                currentUserId = null;
        ...
        /** Genera una descripción de producto usando la API de Gemini con reintentos de backoff. */
        const generateDescription = async (productId, productName, productCategory, buttonEl) => {
            ...
            try {
                ...
                if (text) {
                    ...
                    showModal("Error de IA", "No se pudo generar la descripción. La respuesta de la API fue incompleta o bloqueada.");
                }
        ...
        const deleteProduct = (productId, productName) => {
            showModal(
                ...
                `¿Estás seguro de que deseas eliminar permanentemente el producto "**${productName}**"? Esta acción es inmediata.`,
                ...
                async () => {
                    try {
                        ...
                        showModal("Error de Eliminación", "No se pudo eliminar el producto. Revisa los permisos de Firestore.");
                    }
                }
            );
        };
        ...
        const renderProducts = (products) => {
            ...
            if (products.length === 0) {
                galleryEl.innerHTML = `
                    <div class="col-span-full text-center p-8 bg-white rounded-xl shadow-inner border border-dashed border-gray-300">
                        ...
                        <p class="text-gray-500 mt-2">Usa el botón "Añadir Producto Demo" para comenzar a poblar el catálogo.</p>
                    </div>
                `;
                return;
            }
            ...
            products.forEach(product => {
                ...
                const lastGeneratedTime = product.lastGenerated?.seconds ? new Date(product.lastGenerated.seconds * 1000).toLocaleString() : 'Nunca';
                    ...
                    <div id="product-${product.id}" class="bg-white rounded-3xl shadow-xl hover:shadow-2xl transition duration-300 overflow-hidden border border-gray-100">
                        ...
                        <div class="p-6">
                            <div class="flex justify-between items-start mb-3">
                                ...
                                <span class="text-2xl font-extrabold text-primary">${product.price ? product.price.toFixed(2) : '0.00'}€</span>
                            </div>
                            <p class="text-gray-500 text-sm italic min-h-[40px] mb-4">${descriptionText}</p>
                            <button 
        ...
        const listenToProducts = () => {
            ...
            const q = query(productsCollectionRef); // Se ordena en memoria para evitar errores de índice
            ...
            // En una aplicación real, se devolvería 'unsubscribe' para limpiarlo.
        };
        ...
        const productMocks = [
            ...
            { name: "Libro 'Aventura Criptográfica'", category: "Libros", price: 29.99, imageUrl: "https://placehold.co/400x400/374151/ffffff?text=Libro+Tecnologia" },
        ];
        ...
        addProductBtn.addEventListener('click', async () => {
            ...
            productData.description = ''; // Inicialmente vacío para forzar la generación por IA
            productData.createdAt = serverTimestamp();
            productData.userId = currentUserId; // Quién lo creó (opcional, pero útil)
            ...
            try {
                ...
                showModal("Error", "No se pudo añadir el producto demo a Firestore. Revisa si la colección existe o si tienes permisos de escritura.");
            }
        });
