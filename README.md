...
<body class="bg-gray-100 min-h-screen">
    ...
    <header class="header-nav p-4">
        <div class="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center">
            ...
            <!-- CONTENEDOR DE LOGO 'M' (REVERTIDO A PLACEHOLDER CONFIABLE) -->
            <div class="logo-click-area mb-3 sm:mb-0" onclick="showLogoModal()">
                <img 
                    <!-- REVERTIDO a un placeholder confiable para la 'M' -->
    ...
    <script type="module">
        ...
        /**
         * CORRECCIÓN: Convierte un ID de archivo de Canvas o una URL externa a una URL de origen válida.
         * Si detecta un ID interno (ej. 'uploaded:...'), lo transforma a la ruta accesible.
         */
        const getImageUrl = (urlOrId) => {
            if (!urlOrId) return DEFAULT_IMAGE_URL;
            
            // Los IDs de archivos subidos en Canvas deben tener este prefijo para ser accesibles.
            if (urlOrId.startsWith('uploaded:')) {
                return `/_internal/content/${urlOrId}`;
            }
            
            // Asumir que es una URL externa válida (http/https)
            return urlOrId;
        };
        ...
        const renderProduct = (product) => {
            ...
            if (displayImageUrl && displayImageUrl !== DEFAULT_IMAGE_URL) {
                // Si hay URL corregida, intenta cargarla. Si falla (onerror), muestra el placeholder.
                coverContent = `
                    <img 
                        src="${displayImageUrl}" 
        ...
        const saveProduct = async (e) => {
            ...
            // Guardamos el ID o URL tal como lo introdujo el usuario.
            ...
            try {
                const productData = {
                    title,
                    author,
                    imageUrl, // Guardado sin modificar
                    description,
                    price,
                    userId: userId, 
                };
