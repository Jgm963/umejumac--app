...
<head>
    ...
    <style>
        ...
        #logo-image {
            /* CORRECCIÓN CLAVE: Fondo amarillo para asegurar visibilidad si la imagen falla */
...
<body class="bg-gray-100 min-h-screen">
    ...
    <header class="header-nav p-4">
        <div class="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center">
            ...
            <!-- CONTENEDOR DE LOGO 'M' (Ahora incluye texto "Ver Logo (Pulsa)" MÁS VISIBLE) -->
            <div class="logo-click-area mb-3 sm:mb-0" onclick="showLogoModal()">
                <img 
                    src="uploaded:frt.png-4032f3ef-c19e-4ce6-9539-222a31312e1d" 
        ...
        <section id="products-gallery" class="view">
            ...
            <div id="creation-form-container" class="bg-white p-6 rounded-xl shadow-xl mb-8 hidden fade-in">
                ...
                <form id="product-form" class="space-y-4">
                    ...
                    <!-- CAMPO CLAVE PARA IMAGEN (ACEPTA ID DE ARCHIVO SUBIDO) -->
                    <input type="text" id="product-image-url" 
                        placeholder="Pega aquí el ID o URL de la imagen subida (Libro, Carro, Instrumento - Opcional)" 
    ...
    <script type="module">
        ...
        // ID de la imagen de ejemplo (Tu logo: Imagen de WhatsApp...)
        // Si el usuario no pega una URL, usaremos esta como fallback para el placeholder
        const DEFAULT_IMAGE_ID = "uploaded:Imagen de WhatsApp 2025-10-01 a las 19.35.19_d1a48fa8.jpg-b0f84332-739d-41aa-be4b-59dd0317c5c1";
        ...
        const LOGO_CLICK_AREA = document.querySelector('.logo-click-area'); // Nuevo
        const LOGO_MODAL = document.getElementById('logo-modal');

        // Referencias del Formulario y Galería de Productos (Mantenidas)
        ...
        const SAVE_BUTTON = document.getElementById('save-button'); // Botón de Guardar
        ...
        window.showLogoModal = () => {
            ...
            // Agregar transiciones para la animación de entrada
            setTimeout(() => {
            ...
            try {
                ...
                onAuthStateChanged(auth, (user) => {
                    if (user) {
                        ...
                        // Habilitar el botón de Guardar cuando la sesión está lista
                        ...
                        // Si falla la autenticación, mantener el botón deshabilitado
                ...
                SAVE_BUTTON.disabled = true; // Asegurar que permanezca deshabilitado en caso de error de inicio
            }
        };
        ...
        const renderProduct = (product) => {
            ...
            let coverContent = '';
            // Si no hay URL de imagen, usa el ID de la imagen de ejemplo para el placeholder
            const displayImageUrl = product.imageUrl || DEFAULT_IMAGE_ID; 

            // Se ha mejorado el manejo de imágenes aquí:
            if (displayImageUrl) {
                // Si hay URL, intenta cargarla. Si falla (onerror), muestra el placeholder.
                coverContent = `
                    <img 
                        src="${displayImageUrl}" 
                        alt="Portada de ${product.title}" 
                        class="cover-image rounded-t-xl" 
                        onerror="this.onerror=null; this.outerHTML='<div class=\\"cover-placeholder placeholder-img rounded-t-xl\\"><span>Error cargando imagen: ${product.title}</span></div>';"
                    />
                `;
            } else {
                // Placeholder si no hay URL de imagen ni default
                coverContent = `
        ...
        const hideForm = () => {
            ...
            // Si la sesión está lista, habilitar el botón después del reset
            if (isAuthReady) { 
                SAVE_BUTTON.disabled = false;
            } else {
                // Si no está lista, mantener deshabilitado y mostrar mensaje
                SAVE_BUTTON.disabled = true;
                FORM_ERROR.textContent = 'Error: Sesión no autenticada. No puedes guardar.';
            ...
            // NO restablecemos el valor del input de imagen aquí. Se mantiene en blanco.
        };
        ...
        const startEdit = (product) => {
            ...
            IMAGE_URL_INPUT.value = product.imageUrl || ''; // Cargar URL de imagen existente
        ...
        const saveProduct = async (e) => {
            ...
            // ----------------------------------------------------
            // CORRECCIÓN CLAVE: Verificar isAuthReady y userId aquí también
            // ----------------------------------------------------
            if (!isAuthReady || !userId) {
                // Si el botón está deshabilitado, esto no debería ocurrir, pero es una buena práctica
                // mostrar el error y salir si la sesión se perdió
                FORM_ERROR.textContent = 'Error: Sesión no autenticada. Por favor, recarga la página.';
                FORM_ERROR.classList.remove('hidden');
                SAVE_BUTTON.disabled = true;
                return;
            }
            ...
            // Si el campo de imagen está vacío, se guardará una cadena vacía.
            ...
            try {
                const productData = {
                    title,
                    author,
                    imageUrl, // El campo está aquí para que lo introduzcas manualmente.
                    description,
                    price,
                    userId: userId, 
                };
        ...
        // Evento para abrir el Modal del Logo desde el encabezado
        ...
        // Listener para cerrar el modal del logo al hacer clic fuera (backdrop)
