    ...
    <script type="module">
            ...
            try {
                ...
                onAuthStateChanged(auth, (user) => {
                    if (user) {
                        ...
                        updateSystemStatus(true, 'Modo Dinámico ACTIVO', 'bg-green-600', `ID: ${userId}`); // Muestra el ID completo
                        listenForProducts();
                ...
                isAuthReady = false;
                SAVE_BUTTON.disabled = true;
                updateSystemStatus(false, 'Error de Conexión/Config.', 'bg-red-700', 'Volviendo a Modo Estático');
                loadStaticProducts();
            }
        };
        ...
        const renderProduct = (product) => {
            ...
            card.innerHTML = `
                ...
                <div class="p-4 flex flex-col flex-grow">
                    ...
                    ${(!isStaticMode && product.userId) ? `<p class="text-xs text-gray-400 mb-2">Creador ID: <span class="font-mono break-all">${product.userId}</span></p>` : ''}
        ...
        const showForm = () => {
            ...
            if(isStaticMode || !isAuthReady) {
                 FORM_ERROR.textContent = 'ERROR: El modo dinámico no está activo o la autenticación falló. No se puede acceder al formulario de creación/edición.';
