...
<body class="bg-gray-100 min-h-screen">
    ...
    <script type="module">
        ...
        let isStaticMode = !firebaseConfigExists; // Bandera para la carga en GitHub
        ...
        // --- DATA ESTÁTICA PARA MODO GITHUB PAGES ---
        const STATIC_PRODUCTS = [
            { id: 'static-1', title: 'Autonomy Concept', author: 'Dr. Morales AC', description: 'Publicación clave sobre la autonomía profesional y gestión de riesgos.', price: 29.99, userId: 'STATIC_USER_GH', imageUrl: DEFAULT_IMAGE_URL },
            { id: 'static-2', title: 'Guía Legal Sanitaria 2024', author: 'UMÉJUMAC Editorial', description: 'Manual detallado sobre la normativa sanitaria y su aplicación en la práctica médica.', price: 45.00, userId: 'STATIC_USER_GH', imageUrl: "https://placehold.co/400x180/ccff00/003366?text=NORMATIVA%20LEGAL" },
            { id: 'static-3', title: 'Servicio de Consultoría Premium', author: 'EJMR - Morales Roa', description: 'Acceso a un paquete de 10 horas de consultoría especializada en gestión hospitalaria.', price: 199.99, userId: 'STATIC_USER_GH', imageUrl: "https://placehold.co/400x180/006699/FFFFFF?text=CONSULTORÍA%20VIP" },
        ];
        ...
        const getImageUrl = (urlOrId) => {
            ...
            // Aplicar la corrección solo si estamos en modo dinámico (Canvas)
            if (!isStaticMode && urlOrId.startsWith('uploaded:')) {
                return `/_internal/content/${urlOrId}`;
            }

            // Si es modo estático o una URL externa, se devuelve tal cual.
            return urlOrId;
        };


        const renderProduct = (product) => {
            // En modo estático, todos los productos son del 'usuario estático' y NO se muestran botones de CRUD.
            ...
            // APLICACIÓN DE LA CORRECCIÓN: Resuelve el ID a una URL válida
            const displayImageUrl = getImageUrl(product.imageUrl); 

            let coverContent = '';

            // Renderizar la imagen usando la URL corregida o el placeholder
        ...
        const listenForProducts = () => {
            if (isStaticMode || !db) return; // Salir si es modo estático o no hay db
        ...
        const showForm = () => {
            if(isStaticMode) {
                 // No se muestra el formulario en modo estático
                 return;
            }
        ...
        const saveProduct = async (e) => {
            ...
            if (isStaticMode || !isAuthReady || !userId) {
                FORM_ERROR.textContent = 'Error: La funcionalidad de guardar solo está disponible en el entorno de Canvas.';
        ...
        // Eventos del Formulario de Productos (Solo se asignan si no estamos en modo estático, aunque el control de flujo lo maneja isStaticMode dentro de las funciones)
