...
<head>
    ...
    <style>
        ...
        /* ESTILO MEJORADO PARA LA TARJETA DEL PRODUCTO */
        .book-card {
            box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.1), 0 1px 2px 0 rgba(0, 0, 0, 0.06); /* Sombra ligera inicial */
        ...
        .book-card:hover {
            ...
            box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -4px rgba(0, 0, 0, 0.05); /* Sombra más pronunciada al hacer hover */
        }
        ...
        /* Estilos para el enfoque del formulario */
        .form-input:focus {
            border-color: #003366;
...
<body class="bg-gray-100 min-h-screen">
    ...
    <div id="app-container" class="max-w-7xl mx-auto p-4 sm:p-8">
        ...
        <!-- ====================================
             4. GALERÍA DE PRODUCTOS (VIEW) - MEJORADA
             ==================================== -->
        <section id="products-gallery" class="view">
            <h2 class="text-3xl font-extrabold text-gray-800 mb-6 text-center sm:text-left border-b-4 border-yellow-500 pb-2">Galería de Publicaciones y Servicios Exclusivos</h2>

            <!-- Formulario de Creación Rápida (Inicialmente oculto) -->
            <div id="creation-form-container" class="bg-white p-6 rounded-xl shadow-xl mb-8 hidden fade-in border-t-4 border-blue-600">
                <h3 id="form-title" class="text-2xl font-bold text-gray-800 mb-4 border-b pb-2">Añadir Nuevo Producto (Venta)</h3>
                ...
                <form id="product-form" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="product-title" class="block text-sm font-medium text-gray-700">Título del Producto / Publicación *</label>
                            <input type="text" id="product-title" placeholder="Ej: Autonomy Concept | Equipo Quirúrgico" required class="form-input w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                        </div>
                        <div>
                            <label for="product-author" class="block text-sm font-medium text-gray-700">Autor o Formato (Opcional)</label>
                            <input type="text" id="product-author" placeholder="Ej: EJMR | Modelo 2024" class="form-input w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="product-image-url" class="block text-sm font-medium text-gray-700">ID/URL de Imagen de Portada</label>
                            <input type="text" id="product-image-url" 
                                placeholder="Pega aquí el ID o URL (ej: uploaded:nombre.png-ID)" 
                                title="Para usar una imagen subida: haz clic en el archivo, copia su ID completo y pégalo aquí."
                                class="form-input w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                            <p class="text-xs text-gray-500 mt-1">Si no proporciona una imagen, se usará un fondo azul de UMÉJUMAC.</p>
                        </div>
                        <div>
                            <label for="product-price" class="block text-sm font-medium text-gray-700">Precio (USD) *</label>
                            <input type="number" step="0.01" id="product-price" placeholder="Ej: 25.00" required class="form-input w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                            <p class="text-xs text-gray-500 mt-1">Ingrese el precio sin símbolos monetarios.</p>
                        </div>
                    </div>
                    
                    <!-- Fila 3: Descripción -->
                    <div>
                        <label for="product-description" class="block text-sm font-medium text-gray-700">Descripción Breve *</label>
                        <textarea id="product-description" rows="3" placeholder="Descripción breve del contenido o las características clave del servicio..." required class="form-input w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 resize-none"></textarea>
                    </div>
                    ...
                    <p id="form-error-message" class="text-red-500 text-sm mt-2 hidden text-center"></p>
                </form>
            </div>
    ...
    <script type="module">
        ...
        // (Se mantiene la lógica para gestionar el modo Estático/Dinámico)
        // ===============================================
        ...
        try {
            ...
            if (Object.keys(configData).length > 0 && configData.apiKey) {
                ...
                setLogLevel('Debug');
                console.log("SUCCESS: La configuración de Firebase fue encontrada. Modo DYNAMIC ON (CRUD Habilitado).");
            } else if (typeof __firebase_config !== 'undefined' && __firebase_config.length > 2) {
                 console.warn("WARNING: __firebase_config existe pero parece vacía. Forzando Modo STATIC.");
            } else {
                 console.warn("WARNING: La variable __firebase_config no está definida. Forzando Modo STATIC.");
            }

        } catch (e) {
            console.error("FATAL ERROR: Falló al parsear __firebase_config JSON. Detalles:", e);
        }

        const firebaseConfig = firebaseConfigExists ? configData : {};
        const initialAuthToken = typeof __initial_auth_token !== 'undefined' ? __initial_auth_token : null;
        
        // Define el modo basado en la existencia de la config
        ...
        window.showView = (viewId) => { 
            ...
            if (targetViewId === 'products-gallery') {
                ...
                if (isStaticMode) {
                    ...
                    ADD_BUTTON.textContent = isAuthReady ? '+ Nuevo Producto' : 'Cargando Galería (CRUD)...';
                }
            } else {
        ...
        const STATIC_PRODUCTS = [
            ...
            // NOTA: El ID de la imagen en el modo estático debe coincidir con un archivo real subido
            { id: 'static-1', title: 'Autonomy Concept: Edición 2025', author: 'Dr. Morales AC (EJMR)', description: 'Publicación clave sobre la autonomía profesional y gestión de riesgos médicos y legales en un entorno dinámico.', price: 29.99, userId: 'STATIC_USER', imageUrl: "uploaded:Imagen de WhatsApp 2025-10-01 a las 19.35.19_d1a48fa8.jpg-22918004-4069-4ad5-92ac-1da7a8b99ca6" },
            { id: 'static-2', title: 'Guía Legal Sanitaria 2024', author: 'UMÉJUMAC Editorial', description: 'Manual detallado sobre la normativa sanitaria y su aplicación en la práctica médica y administrativa, incluyendo casos de estudio.', price: 45.00, userId: 'STATIC_USER', imageUrl: "https://placehold.co/400x180/ccff00/003366?text=NORMATIVA%20LEGAL" },
            { id: 'static-3', title: 'Consultoría Premium: 10 horas', author: 'EJMR - Morales Roa', description: 'Acceso a un paquete de 10 horas de consultoría especializada en gestión hospitalaria y defensa legal preventiva.', price: 199.99, userId: 'STATIC_USER', imageUrl: "https://placehold.co/400x180/006699/FFFFFF?text=CONSULTORÍA%20VIP" },
            { id: 'static-4', title: 'Ética y Responsabilidad Médica', author: 'Especialistas en Derecho', description: 'Libro sobre los desafíos éticos y la responsabilidad civil y penal en el campo de la salud colectiva.', price: 35.50, userId: 'STATIC_USER', imageUrl: "https://placehold.co/400x180/6366f1/FFFFFF?text=ÉTICA%20MÉDICA" },
        ];
        ...
        const loadStaticProducts = () => {
            ...
            STATIC_PRODUCTS.forEach(product => {
             ...
             PRODUCTS_CONTAINER.innerHTML += '<p class="text-center text-orange-600 col-span-full mt-4 p-4 border border-orange-200 rounded-lg bg-orange-50 font-medium">Esta es una **Galería Estática** para visualización. El botón de edición/agregar está deshabilitado por el modo de solo lectura.</p>';
        };

        
        // --- 3. Renderizado de Productos y Lógica de Imagen ---
        
        const getImageUrl = (urlOrId) => {
            if (!urlOrId) return DEFAULT_IMAGE_URL;
            
            // Corrige la ruta si es un archivo subido en el entorno Canvas
            if (urlOrId.startsWith('uploaded:')) {
                // Utiliza la ruta interna para acceder a los archivos subidos
        ...
        const renderProduct = (product) => {
            ...
            let coverContent = `
                <img 
                    ...
                    onerror="this.onerror=null; this.src='${DEFAULT_IMAGE_URL}'; this.outerHTML='<div class=\\"cover-placeholder placeholder-img rounded-t-xl\\"><span>IMAGEN NO DISPONIBLE: ${product.title || 'N/A'}</span></div>';"
                />
            `;
            ...
            card.innerHTML = `
                ...
                <div class="p-4 flex flex-col flex-grow">
                    <!-- Título -->
                    <h4 class="text-lg font-bold text-blue-800 mb-1 leading-tight">${product.title || 'Título del Producto'}</h4>
                    <!-- Autor / Formato -->
                    <p class="text-xs text-gray-500 mb-2">${product.author || 'Autor/Formato: N/A'}</p>
                    
                    <!-- Descripción -->
                    <p class="text-sm text-gray-700 flex-grow mb-4 line-clamp-3">${product.description || 'Descripción: Reemplace este texto con información detallada.'}</p>
                    
                    <!-- Precio Destacado -->
                    <div class="mt-auto pt-2 border-t border-gray-100">
                        <p class="text-xl font-extrabold text-green-700">USD ${parseFloat(product.price).toFixed(2)}</p>
                    </div>

                    <!-- Botones de Acción (Solo si es el propietario y NO es modo estático) -->
                    ${(isOwner && !isStaticMode) ? `
                        <div class="flex mt-3 space-x-2">
                            <button class="edit-btn flex-1 text-xs font-semibold py-1.5 px-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition shadow-md">
                                Editar
                            </button>
                            <button class="delete-btn flex-1 text-xs font-semibold py-1.5 px-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition shadow-md">
                                Eliminar
                            </button>
                        </div>
                    ` : ''}
        ...
        // --- 5. Lógica de CRUD y Modal ---
        
        const showForm = () => {
            if(isStaticMode || !isAuthReady) {
                 FORM_ERROR.textContent = 'ERROR: La aplicación está en modo estático (solo lectura) o la conexión a la base de datos no es válida. No se puede acceder al formulario de creación/edición.';
        ...
        const hideForm = () => {
            ...
            FORM_TITLE.textContent = 'Añadir Nuevo Producto (Venta)';
            if (isAuthReady) { 
                SAVE_BUTTON.disabled = false;
            } else {
                SAVE_BUTTON.disabled = true;
        ...
        const startEdit = (product) => {
            ...
            FORM_TITLE.textContent = 'Editar Producto Existente';
        ...
        const saveProduct = async (e) => {
            ...
            if (!title || !description || isNaN(price) || price < 0) {
                FORM_ERROR.textContent = 'Por favor, rellena los campos obligatorios de Título, Descripción y Precio de forma válida.';
                FORM_ERROR.classList.remove('hidden');
                return;
            }
            
            SAVE_BUTTON.disabled = true;
            SAVE_BUTTON.textContent = isEditing ? 'Actualizando...' : 'Guardando...';
            FORM_ERROR.classList.add('hidden');
            ...
            try {
                ...
                SAVE_BUTTON.disabled = false;
                SAVE_BUTTON.textContent = isEditing ? 'Actualizar Producto' : 'Guardar Producto';
            }
        };
        ...
        const showConfirmModal = (product) => {
            ...
            document.getElementById('modal-text').textContent = `¿Estás seguro de que quieres eliminar el producto: "${product.title}"?`;
            MODAL_ELEMENT.classList.remove('hidden');
            MODAL_ELEMENT.classList.add('flex', 'fade-in');
        };

        const hideConfirmModal = () => {
            MODAL_ELEMENT.classList.add('hidden');
            MODAL_ELEMENT.classList.remove('flex', 'fade-in');
            productToDelete = null;
        };

        const deleteProduct = async () => {
            if (!productToDelete || isStaticMode || !isAuthReady) return;

            CONFIRM_BUTTON.disabled = true;
            CONFIRM_BUTTON.textContent = 'Eliminando...';
            ...
            try {
                ...
                CONFIRM_BUTTON.textContent = 'Eliminar';
            }
        };
        ...
        MODAL_ELEMENT.addEventListener('click', (e) => {
            if (e.target === MODAL_ELEMENT) hideConfirmModal();
        });

        // --- 6. Inicialización de la Aplicación (Start) ---
        const startApp = () => {
            initializeFirebase();
            showView('main');
        };
