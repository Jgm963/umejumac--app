...
<body class="bg-gray-100 min-h-screen">
    ...
    <script type="module">
        ...
        // Importaciones corregidas para CRUD: addDoc (para agregar), updateDoc (para editar), deleteDoc (para eliminar)
        ...
        // const NAV_ELEMENTS = document.querySelectorAll('.nav-link'); // No usados directamente, usamos los eventos en las tarjetas
        // const CARD_ELEMENTS = document.querySelectorAll('.main-section-card'); // Se usa para la navegación en la sección principal
        ...
        window.showView = (viewId) => { 
            ...
            if (targetViewId === 'products-gallery' && isStaticMode) {
                ...
                // Si la galería está vacía en modo estático, cargar los productos de ejemplo
                if (PRODUCTS_CONTAINER.children.length === 0 || PRODUCTS_CONTAINER.querySelector('#empty-message')) {
                     // Solo mostrar el mensaje de carga estática si no se están mostrando productos dinámicos
                     if (!db) { 
        ...
        const getImageUrl = (urlOrId) => {
            ...
            if (urlOrId.startsWith('uploaded:')) {
                // IMPORTANT: Use the /_internal/content/ prefix to access uploaded files
        ...
        const renderProduct = (product) => {
            ...
            if (isOwner && !isStaticMode) {
                // Usamos querySelectorAll para asegurarnos de adjuntar el evento incluso si la tarjeta se actualiza
        ...
        const showForm = () => {
            ...
            // Desplazar la vista al formulario para que el usuario pueda verlo
        ...
        const hideForm = () => {
            ...
            PRODUCT_FORM.reset();
            isEditing = false;
            EDITING_PRODUCT_ID_INPUT.value = ''; // Limpiar el ID de edición
        ...
        const startEdit = (product) => {
            ...
            isEditing = true;
            EDITING_PRODUCT_ID_INPUT.value = product.id; // Establecer el ID de edición
            ...
            // Aseguramos que el precio sea una cadena con dos decimales para el input number
        ...
        const saveProduct = async (e) => {
            ...
            try {
                ...
                if (isEditing && editingProductId) {
                    ...
                    // 2. AGREGAR: addDoc (CORREGIDO)
                    const productsRef = collection(db, PRODUCTS_COLLECTION_PATH);
                    await addDoc(productsRef, {
                        ...productData,
                        createdAt: serverTimestamp(), // Agregar marca de tiempo de creación
                    });
                }
        ...
        // Eventos para la navegación de la barra superior
        ...
        // Eventos para la navegación de las tarjetas principales (main-view)
        document.querySelectorAll('.main-section-card').forEach(card => {
             card.addEventListener('click', (e) => {
                 // Aseguramos que el click en el botón dentro de la tarjeta no duplique la navegación
