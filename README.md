    ...
    <script type="module">
        ...
        const initializeFirebase = async () => {
            try {
                ...
                if (!firebaseConfig) {
                    throw new Error("Configuración de Firebase no disponible. Activando modo estático.");
                }
                ...
                onAuthStateChanged(auth, (user) => {
                    if (user) {
                        ...
                        // Esto solo debería ocurrir si el token es inválido o expira
                        userId = 'anonimo';
                        authStatus.textContent = `Error: Sesión no autenticada. Activando modo estático.`;
                        authStatus.classList.remove('text-green-600', 'bg-green-100');
                        ...
                        displayStaticProducts(); // Mostrar productos estáticos si falla la auth
                    }
                });

            } catch (error) {
                console.error("Error al inicializar o autenticar Firebase:", error);
                authStatus.textContent = `ERROR CRÍTICO: ${error.message}`;
                authStatus.classList.remove('hidden');
                salesButton.textContent = 'Galería Estática (Solo Lectura)';
                salesButton.disabled = true;
                loadingSpinner.classList.add('hidden');
        ...
        const loadProductGallery = (appId) => {
            ...
            onSnapshot(productsRef, (snapshot) => {
                ...
                authStatus.classList.remove('text-red-600', 'bg-red-100', 'hidden');
                authStatus.classList.add('text-green-600', 'bg-green-100');
            }, (error) => {
                console.error("Error en onSnapshot (Firestore):", error);
                // Si la conexión falla después de la autenticación
        ...
        const createProductCard = (product) => {
            ...
            // Determinar la fuente de la imagen (debe ser un ID de archivo subido)
            const imageId = product.image.startsWith('uploaded:') ? product.image : 'https://placehold.co/400x300/4F46E5/ffffff?text=IMG+FALTA';

            card.innerHTML = `
                <!-- Imagen -->
                <div class="h-48 overflow-hidden bg-gray-200">
                    <img src="${imageId}" alt="${product.title}" class="w-full h-full object-cover">
                </div>
                <!-- Contenido -->
        ...
        window.deleteProduct = async (productId) => {
            ...
            // NO USAR confirm() - Usar un modal personalizado si fuera necesario.
        ...
        window.toggleProductModal = () => {
            ...
            if (modal.classList.contains('hidden')) {
                ...
                if (auth.currentUser) {
                    ...
                    console.warn("No se puede abrir el modal: Sesión no autenticada.");
                }
            } else {
